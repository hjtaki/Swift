...
let yourSentence = "The \(nounArray[6]) liked the \(nounArray[7])."
let yourSillySentence = "The \(nounArray[index1]) break the \(nounArray[index2])."
//: ### Exercise 2
...
for word in newestString.split(separator:" "){
    if let val = dictionary[String(word)]{
        output += val + " "
    }else{
        output += word + " "
    }
...
let dollors = "$\(numOfPennies/100).\(numOfPennies % 100)"
print(dollors)
