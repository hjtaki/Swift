...
for word in nonsenseArray{
    print(word[word.index(before: word.endIndex)],terminator : "")
}
//: __Problem 3__
//:
...
func checkIfNumeric(input : String) ->Bool{
    if (Int(input) != nil){
        return true
    }
    return false
...
//checkIfNumeric(input: digits.uni)
...
class MovieArchive {
    ...
    func filterByDirector(array: [String:String], director : String) -> [String] {
        var newArray:[String] = []
        
        for arr in array{
        if(arr.value == director){
            newArray.append(arr.key)
            }
        }
        print(newArray)
        return newArray
...
movie.filterByDirector(array: movies, director: "Ava Du Vernay")
